...
import { processFileBuffer } from "../src/lib/processFile.js";
...
export default async function handler(req, res) {
  ...
  if (!serviceClient) return res.status(500).json({ error: "server misconfigured" });

  const user = await getUser(serviceClient, req);
  if (!user) return res.status(401).json({ error: "unauthorized" });

  const { action, filename, storagePath } = req.body || {};
  ...
  if (action === "get-url") {
    ...
    return res.status(200).json({ signedUrl: data.signedUrl, token: data.token, storagePath: path });
  }
