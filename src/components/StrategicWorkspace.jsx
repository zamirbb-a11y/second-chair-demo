...
function WhatChanged({ analysisDiff, workspaceUpdates }) {
  ...
  return (
    <div className="space-y-2">
      {primaryChange && (
        <CollapsibleChangeCard
          change={primaryChange}
          label="שינוי מרכזי בניתוח"
          primary
        />
      )}
      ...
      {hasUpdates && (
        <div className="space-y-2">
          ...
          {workspaceUpdates.slice(0, 4).map((update, index) => (
            <CollapsibleUpdateCard key={index} update={update} />
          ))}
...
function CollapsibleChangeCard({ change, label, primary = false }) {
  ...
  return (
    <div
      className={`rounded-2xl border bg-white shadow-sm overflow-hidden ${
        primary
          ? "border-blue-300 border-r-4 border-r-blue-500"
          : "border-white/70"
      }`}
    >
      <button
            ...
            <div className="flex items-center gap-2 mb-1">
              <span
                className={`text-xs font-semibold ${
                  primary ? "text-blue-700" : "text-slate-400"
                }`}
              >
                {label}
              </span>
...
function CollapsibleUpdateCard({ update }) {
  const [open, setOpen] = useState(false);

  return (
    <div className="rounded-xl border border-white/70 bg-white shadow-sm overflow-hidden">
      <button
        onClick={() => setOpen((prev) => !prev)}
        className="w-full text-right p-2.5 hover:bg-slate-50 transition"
      >
        <div className="flex items-start gap-2">
...
function ChangeBadge({ type }) {
  if (type === "battlefield-shift") {
    return (
      <span className="mt-0.5 shrink-0 rounded-full bg-purple-50 border border-purple-100 text-purple-700 px-2 py-0.5 text-[11px]">
        מוקד השתנה
      </span>
    );
  }
  ...
  if (type === "issue-shift") {
    return (
      <span className="mt-0.5 shrink-0 rounded-full bg-amber-50 border border-amber-100 text-amber-700 px-2 py-0.5 text-[11px]">
        סוגיה השתנתה
      </span>
    );
  }
  ...
  return (
    <span className="mt-0.5 shrink-0 rounded-full bg-slate-100 border text-slate-600 px-2 py-0.5 text-[11px]">
      שינוי
    </span>
  );
}
