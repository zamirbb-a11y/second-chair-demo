...
export default function AnalysisWorkspace({
  ...
  const workstreams = [
    ...
    {
      title: "מסמכים",
      items: ac?.discoveryTargets || eg?.missingEvidence || [],
  ...
  return (
    <div className="space-y-8">
      ...
      {(analysisDiff.length > 0 || workspaceUpdates.length > 0) && (
      ...
      <section className="space-y-4">
        ...
        <div className="bg-slate-100 border border-slate-200 rounded-3xl overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-5 border-b border-slate-200 bg-slate-50">
              ...
              <button
                ...
                className={
                  activeSection === section.id
                    ? `text-right px-4 py-3 bg-white shadow-sm border-t-4 ${section.color}`
...
/>
      ...
      <section className="space-y-4">
        ...
        <div className="bg-amber-50/40 border border-amber-100 rounded-3xl p-5">
          ...
          <div className="mt-5">
            <Card title="הוסף מידע חדש" compact>
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-dashed bg-white px-4 py-5 text-sm text-slate-500">
                  העלאת קבצים — רכיב להשלמה
                </div>

                <div className="flex justify-between items-center">
                  <div className="text-xs text-slate-500">
                    המערכת תסווג את המידע באופן אוטומטי.
                  </div>

                  <button
                    onClick={submitFreeText}
                    disabled={!freeText.trim()}
                    className="rounded-xl bg-slate-900 text-white px-5 py-2.5 text-sm font-semibold disabled:opacity-50"
                  >
                    עדכן את הניתוח
                  </button>
                </div>
...
function getSectionSummary(id, { snapshot, ct, eg }) {
  if (id === "case") {
    return snapshot?.riskLevel
      ? `סיכויי התיק: ${snapshot.riskLevel}`
      : "מוקד וסיכון";
  }
...
function CaseDetails({ ev, snapshot }) {
  const criticalIssues = (ev?.criticalIssues || []).slice(0, 2);
  const secondaryIssues = (ev?.criticalIssues || []).slice(2, 5);
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
      ...
      <InfoCard title="סוגיות מרכזיות">
        <div className="space-y-4">
          {!criticalIssues.length && (
            <div className="text-sm text-slate-500">
              לא זוהו סוגיות מרכזיות.
            </div>
          )}
      ...
      <InfoCard title="מחלוקות נוספות">
        <div className="space-y-4">
          {!secondaryIssues.length && (
            <div className="text-sm text-slate-500">
              לא זוהו מחלוקות נוספות.
            </div>
          )}

          {secondaryIssues.map((item, index) => (
            <div
              key={index}
              className="pb-4 border-b border-slate-100 last:border-b-0 last:pb-0"
            >
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      <InfoCard title="תיאוריית התובע">
        <CompactList items={ct?.claimantTheory?.points} />
      </InfoCard>

      <InfoCard title="תיאוריית ההגנה">
        <CompactList items={ct?.defenseTheory?.points} />
      </InfoCard>
    </div>
  );
}
...
function EvidenceDetails({ eg }) {
  return (
    <div className="space-y-3">
      {(eg?.evidenceMap || []).slice(0, 4).map((row, index) => (
        <div
          key={index}
      ...
      {precedents.map((p, index) => (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="text-[11px] rounded-full bg-amber-100 text-amber-800 px-2 py-1 shrink-0">
              {String(p.helps || "Mixed")}
            </div>
          </div>
...
function Workstream({
  ...
  return (
    <section className={`bg-white border rounded-2xl p-4 ${color}`}>
        ...
        <div className="space-y-2">
          {rows.map((item, index) => (
            <button
              ...
              className={
                placeholder
                  ? "w-full text-right rounded-xl border border-dashed border-slate-200 bg-slate-50 hover:bg-white p-3 text-sm leading-6 text-slate-500 transition"
                  : "w-full text-right rounded-xl border border-slate-200 bg-slate-50 hover:bg-white p-3 text-sm leading-6 transition"
              }
            >
              {item}
            </button>
...
function SmallItemDrawer({ item, onClose, onAddWorkspaceUpdate }) {
...
function Feed({ workspaceUpdates, analysisDiff }) {
  const items = [
    ...analysisDiff.map((item) => ({
      type: "עדכון ניתוח",
      text: safeText(item?.text || item),
    })),

    ...workspaceUpdates.map((item) => ({
      type: safeText(item?.type || "עדכון"),
