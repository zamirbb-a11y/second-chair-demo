    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <span className="text-xs text-slate-500">
              ביטחון: {analysis?.confidence || "Medium"}
            </span>
          </div>
        </div>
      </section>
      ...
      {(analysisDiff.length > 0 || workspaceUpdates.length > 0) && (
        <section className="border rounded-2xl bg-slate-50 overflow-hidden">
          ...
          >
            <div>
              <div className="font-semibold text-sm">עדכוני תיק</div>
      ...
      <section>
        ...
        <div className="bg-white border rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="p-5 bg-slate-50/50">
            {activeSection === "case" && (
              <CaseDetails ev={ev} snapshot={snapshot} outlook={outlook} />
            )}
      ...
      <section>
        ...
        <div className="mt-4">
          <Card title="הוסף מידע חדש לתיק" compact>
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center gap-3">
                <div className="text-xs text-slate-500">
                  לאחר ההוספה ניתן להריץ ניתוח מחדש.
                </div>
...
function getSectionSummary(id, { snapshot, ct, eg }) {
  if (id === "case") return snapshot?.riskLevel || "סיכון ומוקד";
  if (id === "theory") return ct?.litigationBattleground?.issue || "עילות והגנות";
  if (id === "evidence") return eg?.missingEvidence?.[0] || "ראיות וחוסרים";
  if (id === "timeline") return "כרונולוגיה";
  return "";
}

function CaseDetails({ ev, snapshot, outlook }) {
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
      <DetailBox title="מוקד המחלוקת">
        <p>{snapshot?.coreDispute || "לא זוהה"}</p>
        <GroundingInline items={snapshot?.grounding} />
      </DetailBox>

      <DetailBox title="סוגיות מרכזיות">
        <div className="space-y-3">
          {(ev?.criticalIssues || []).slice(0, 3).map((item, index) => (
            <div key={index}>
      ...
      <DetailBox title="הערכת מצב">
        <p className="font-medium">זירת הקרב המרכזית</p>
        <p className="mt-2">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
      <DetailBox title="תיאוריית התובע">
        <p className="font-medium">
          {ct?.claimantTheory?.headline || "לא זוהתה"}
        </p>
        <CompactList items={ct?.claimantTheory?.points} limit={4} />
      </DetailBox>

      <DetailBox title="תיאוריית ההגנה">
        <p className="font-medium">
          {ct?.defenseTheory?.headline || "לא זוהתה"}
        </p>
        <CompactList items={ct?.defenseTheory?.points} limit={4} />
      </DetailBox>
  ...
  return (
    <div className="space-y-3">
      {rows.slice(0, 5).map((row, index) => (
        ...
        >
          <div>
            <div className="text-xs text-slate-500 mb-1">סוגיה</div>
            <div className="font-semibold">{row.issue}</div>
          </div>

          <div>
            <div className="text-xs text-slate-500 mb-1">ראיה קיימת</div>
            <div className="text-sm leading-6">{row.existingEvidence}</div>
          </div>

          <div>
            <div className="text-xs text-slate-500 mb-1">חסר / דורש השלמה</div>
            <div className="text-sm leading-6">{row.missingEvidence}</div>
          </div>
...
function Workstream({ title, items, empty, placeholder, onSelect }) {
...
function ItemDrawer({ item, onClose, onAddWorkspaceUpdate }) {
      ...
      <div className="bg-white rounded-2xl shadow-xl border w-full max-w-2xl p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h3 className="font-semibold leading-7">{item.text}</h3>
          </div>
        ...
        <div className="mt-3 rounded-2xl border border-dashed bg-slate-50 p-4 text-sm text-slate-500">
          העלאת קבצים לפריט הספציפי — רכיב להשלמה בשלב הבא.
        </div>
