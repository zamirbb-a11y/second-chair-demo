...
exports.processFileBuffer = processFileBuffer;
...
async function processFileBuffer(buffer, filename) {
  ...
  let needsOcr = false;
  if (extension === "docx") {
    const result = await _mammoth.default.extractRawText({
      buffer
    });
    extractedText = result.value || "";
    extractionMethod = "mammoth";
  } else if (extension === "pdf") {
    const result = await (0, _pdfParse.default)(buffer);
...
function buildDocumentProfile({
  ...
  const pleadingType = inferPleadingType({
    lower
  });
  const proceduralRole = inferProceduralRole({
    lower
  });
...
function inferDocumentRole({
  ...
  if (lower.includes("spa") || lower.includes("share purchase") || lower.includes("asset purchase") || lower.includes("הסכם רכישה") || lower.includes("הסכם מכר") || lower.includes("מצגים והתחייבויות")) {
    return "הסכם / מסמך חוזי מרכזי";
  }
  if (extension === "eml" || lower.includes("subject:") || lower.includes("from:") || lower.includes("to:")) {
    return "תכתובת אימייל";
  }
  if (lower.includes("whatsapp") || lower.includes("וואטסאפ") || lower.includes("תמלול הודעות") || lower.includes("הודעה")) {
    return "תכתובת הודעות / WhatsApp";
  }
  if (lower.includes("board") || lower.includes("דירקטוריון") || lower.includes("פרוטוקול") || lower.includes("minutes")) {
    return "פרוטוקול / מסמך הנהלה";
  }
  if (lower.includes("due diligence") || lower.includes("dd") || lower.includes("בדיקת נאותות")) {
    return "בדיקת נאותות / DD";
  }
  if (lower.includes("draft") || lower.includes("טיוטה")) {
    return "טיוטה / גרסה מוקדמת";
  }
  if (lower.includes("כתב תביעה") || lower.includes("כתב הגנה") || lower.includes("בקשה") || lower.includes("תצהיר")) {
...
function inferDocumentKind({
  ...
  if (lower.includes("כתב תביעה") || lower.includes("כתב הגנה") || lower.includes("כתב תשובה") || lower.includes("תשובה") || lower.includes("תגובה לתשובה") || lower.includes("בקשה") || lower.includes("תגובה לבקשה") || lower.includes("תצהיר") || lower.includes("ערעור")) {
    return "pleading";
  }
  if (lower.includes("פסק דין") || lower.includes('פס"ד') || lower.includes("עא ") || lower.includes("רעא ") || lower.includes("תא ") || lower.includes('ת"א ') || lower.includes('תא"מ')) {
    return "precedent";
  }
  if (lower.includes("החלטה") || lower.includes("ניתנה היום") || lower.includes("בית המשפט") || lower.includes("השופט") || lower.includes("השופטת") || lower.includes("הרשם")) {
    return "judicial_decision";
  }
  if (lower.includes("הסכם") || lower.includes("agreement") || lower.includes("contract") || lower.includes("spa")) {
    return "contract";
  }
  if (extension === "eml" || lower.includes("subject:") || lower.includes("from:") || lower.includes("to:")) {
  ...
  if (lower.includes("פרוטוקול") || lower.includes("דירקטוריון") || lower.includes("board")) {
...
function extractLegalReferences(text) {
  ...
  const patterns = [/סעיף\s+\d+[א-ת]?(?:\([^)]+\))?\s+ל[^,\n.]{2,80}/g, /חוק\s+[^,\n.]{2,80}/g, /פקודת\s+[^,\n.]{2,80}/g, /תקנה\s+\d+[א-ת]?(?:\([^)]+\))?/g];
  for (const pattern of patterns) {
...
function createFileId(name = "") {
  return "file_" + name.toLowerCase().replace(/[^a-z0-9א-ת]+/gi, "_").replace(/^_+|_+$/g, "").slice(0, 60) + "_" + Math.random().toString(36).slice(2, 8);
}
