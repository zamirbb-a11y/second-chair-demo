...
exports.formatPrecedentsForPrompt = formatPrecedentsForPrompt;
exports.retrieveRelevantPrecedents = retrieveRelevantPrecedents;
function normalizeText(value) {
  return String(value || "").toLowerCase().replace(/[^\u0590-\u05FFa-zA-Z0-9\s]/g, " ").replace(/\s+/g, " ").trim();
}
...
function precedentText(precedent) {
  return [precedent.title, precedent.shortName, precedent.court, ...(precedent.issues || []), ...(precedent.statutes || []), ...(precedent.factualContext || []), precedent.holding, precedent.risks, precedent.keyQuotes, precedent.miniRatio].filter(Boolean).join("\n");
}
function retrieveRelevantPrecedents(caseText, precedents = [], limit = 6) {
  ...
  const scored = precedents.map(precedent => {
    let score = 0;
    const reasons = [];
    const issueMatches = countMatches(caseTokens, (precedent.issues || []).join(" "));
    if (issueMatches > 0) {
      score += issueMatches * 5;
      reasons.push(`התאמה לסוגיות/תגיות (${issueMatches})`);
    }
    const statuteMatches = countMatches(caseTokens, (precedent.statutes || []).join(" "));
    if (statuteMatches > 0) {
      score += statuteMatches * 4;
      reasons.push(`התאמה לחקיקה (${statuteMatches})`);
    }
    const titleMatches = countMatches(caseTokens, `${precedent.title || ""} ${precedent.shortName || ""}`);
    if (titleMatches > 0) {
  ...
  return scored.filter(item => item.retrievalScore > 0).sort((a, b) => b.retrievalScore - a.retrievalScore).slice(0, limit);
}
function formatPrecedentsForPrompt(precedents = []) {
...
//# sourceMappingURL=precedentRetrieval.js.map