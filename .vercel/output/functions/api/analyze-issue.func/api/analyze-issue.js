...
`;
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "system",
        content: `אתה עורך דין ישראלי מנוסה המייצג את הצד שכנגד (${theirLabel}). תפקידך לתקוף את עמדת ${ourLabel} ולחשוף חולשות אמיתיות בלבד — לא להמציא. החזר JSON תקין בלבד.`
      }, {
        role: "user",
...
async function handler(req, res) {
  ...
  try {
    ...
    if (!issue?.title) return res.status(400).json({
      error: "Missing issue"
    });
...
${JSON.stringify((liveCaseState?.issues || []).map(i => ({
      id: i.id,
      title: i.title,
...
`;
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "system",
          content: "אתה עורך דין ישראלי בכיר בליטיגציה מסחרית. תפקידך לנתח מחלוקת ספציפית אחת מול חומר תיק קיים. אל תחרוג מהמחלוקת שצוינה. החזר JSON תקין בלבד."
        }, {
          role: "user",
          content: prompt
        }],
...
//# sourceMappingURL=analyze-issue.js.map