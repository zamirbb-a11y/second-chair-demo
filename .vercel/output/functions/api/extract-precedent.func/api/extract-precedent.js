...
var _normalizeIssues = require("../src/lib/normalizeIssues.js");
...
`;
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "system",
          content: "אתה מומחה לחילוץ מידע מובנה מפסקי דין ישראליים. החזר JSON תקין בלבד."
        }, {
          role: "user",
          content: prompt
        }]
    ...
    if (!content) {
      return res.status(500).json({
        error: "No extraction content returned"
      });
    }
    let parsed;
    try {
    ...
    const normalizedPrecedent = {
      ...precedent,
      normalizedIssues: (0, _normalizeIssues.normalizeIssues)([...(precedent.issues || []), ...(precedent.legalWorlds || []), ...(precedent.nevoTags || []), precedent.miniRatio || "", precedent.holding || ""])
    };
...
function normalizePrecedent(raw, fileName) {
  ...
  const displayName = raw.displayName || `${caseNumber} ${partyNames}`.trim() || fileName || "";
  return {
...
function createPrecedentId(value = "") {
  const base = value.toLowerCase().replace(/[^a-z0-9א-ת]+/gi, "_").replace(/^_+|_+$/g, "").slice(0, 80);
  return `precedent_${base}_${Math.random().toString(36).slice(2, 8)}`;
}
//# sourceMappingURL=extract-precedent.js.map