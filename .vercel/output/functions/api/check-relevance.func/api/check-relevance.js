...
async function handler(req, res) {
  ...
  try {
    ...
    if (!item?.title || !issues.length) return res.status(200).json({
      relevantIssueIds: []
    });
    const issueList = issues.map(i => `ID: "${i.id}" — ${i.title}${i.description ? `: ${i.description}` : ""}`).join("\n");
...
אם הפריט לא רלוונטי לשום מחלוקת — החזר מערך ריק.`;
    ...
    return res.status(200).json({
      relevantIssueIds: parsed.relevantIssueIds ?? []
    });
  } catch (err) {
...
//# sourceMappingURL=check-relevance.js.map