...
var _buildAnalyzePrompt = _interopRequireDefault(require("../src/prompts/buildAnalyzePrompt"));
var _contractFormationDefects = _interopRequireDefault(require("../src/legal-packs/contractFormationDefects"));
...
`;
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "system",
        content: `אתה עורך דין יריב ישראלי מנוסה. תפקידך לתקוף את עמדת ${clientLabel} בכל מחלוקת ולחשוף חולשות אמיתיות בלבד. החזר JSON תקין בלבד.`
      }, {
        role: "user",
        content: prompt
      }],
...
async function handler(req, res) {
  ...
  try {
    ...
    const fullCaseText = [caseText, documentText, ...(files || []).map(file => file?.text || "")].filter(Boolean).join("\n\n");
    const retrievedPrecedents = (0, _precedentRetrieval.retrieveRelevantPrecedents)(fullCaseText, _precedents.default, 6);
    console.log("Loaded precedents:", _precedents.default.length);
    console.log("Retrieved precedents:", retrievedPrecedents.map(p => p.shortName || p.title));
    const prompt = (0, _buildAnalyzePrompt.default)({
      caseText,
      documentText,
      files,
      legalPacks: [_contractFormationDefects.default],
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "system",
          content: "אתה עורך דין ישראלי בכיר בדיני חוזים וליטיגציה מסחרית. אתה בונה cockpit ליטיגטורי: עובדות, ראיות, סיכונים, תיאוריות תיק, עילות רלוונטיות, סעדים וצעדים הבאים. כאשר מסופקים לך מקורות משפטיים פנימיים, השתמש בהם בלבד כמקורות פסיקה. אל תמציא פסיקה. אם אין מקור מתאים, כתוב שחסר מקור משפטי מתאים במאגר."
        }, {
          role: "user",
          content: prompt
        }],
    ...
    try {
      ...
      const mergedPrecedents = gptPrecedents.map(gptP => {
        const bankMatch = _precedents.default.find(b => b.caseNumber && gptP.caseNumber && b.caseNumber === gptP.caseNumber) ?? _precedents.default.find(b => gptP.name && (b.shortName?.toLowerCase() === gptP.name.toLowerCase() || b.title?.toLowerCase().includes(gptP.name.toLowerCase())));
        if (bankMatch) {
          return {
            ...bankMatch,
      ...
      console.log("Merged precedents:", mergedPrecedents.map(p => `${p.shortName} (${p.helps})`));
      const effectiveClientRole = parsed.clientRole ?? "claimant";
      let adversarialReviews = {};
      try {
        adversarialReviews = await runAdversarialPass(parsed.issues ?? [], caseText, clientName, effectiveClientRole);
      } catch (e) {
