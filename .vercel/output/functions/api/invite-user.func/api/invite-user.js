...
async function handler(req, res) {
  ...
  if (!serviceKey) return res.status(500).json({
    error: "server misconfigured: service key missing"
  });
  if (!gmailUser || !gmailPass) return res.status(500).json({
    error: "server misconfigured: gmail credentials missing"
  });
  const serviceClient = (0, _supabaseJs.createClient)(supabaseUrl, serviceKey, {
    auth: {
  ...
  try {
    ...
    return res.status(500).json({
      error: "שליחת המייל נכשלה: " + mailErr.message
    });
  }
